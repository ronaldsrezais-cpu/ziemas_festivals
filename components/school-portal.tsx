...
export function SchoolPortal() {
  ...
  useEffect(() => { load(); }, [load]);
  if (loading) return <main className="mx-auto grid min-h-[60vh] max-w-5xl place-items-center px-4"><Loader2 className="size-10 animate-spin text-[#008baa]"/></main>;
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
    ...
    <section className="mb-7 grid gap-4 md:grid-cols-3">
      ...
      <div className="glass-panel flex flex-wrap content-center gap-2 rounded-3xl p-5 md:col-span-1"><Button asChild className="bg-[#07152f]"><Link href="/skolai/drosibas-lapa"><FileSignature/> Drošības parakstu lapa</Link></Button><Button asChild variant="outline"><Link href="/skolai/akreditacijas"><Download/> Akreditācijas</Link></Button></div>
    </section>
    ...
    <Tabs defaultValue="participants">
      <TabsList className="mb-6 h-auto rounded-2xl bg-[#07152f] p-1.5 text-white"><TabsTrigger value="participants" className="min-h-11 px-5 data-[state=active]:bg-[#f7d21f] data-[state=active]:text-[#07152f]">Dalībnieki</TabsTrigger><TabsTrigger value="leaders" className="min-h-11 px-5 data-[state=active]:bg-[#f7d21f] data-[state=active]:text-[#07152f]">Komandas vadītāji</TabsTrigger></TabsList>
      <TabsContent value="participants">
        <div className="mb-4 flex justify-end"><Dialog open={participantOpen} onOpenChange={setParticipantOpen}><DialogTrigger asChild><Button className="bg-[#07152f]"><Plus/> Pievienot dalībnieku</Button></DialogTrigger><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>Pievienot dalībnieku</DialogTitle><DialogDescription>Sistēma piedāvās tikai dzimšanas gadam un dzimumam atbilstošās kategorijas.</DialogDescription></DialogHeader><ParticipantForm sports={data.sports} onSave={async (payload) => { await action({ action: "save-participant", ...payload }); setParticipantOpen(false); }} /></DialogContent></Dialog></div>
