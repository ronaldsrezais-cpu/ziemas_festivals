...
export function SchoolPortal() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
    ...
    {!data.rosterEditable && <p className="mb-5 rounded-2xl bg-amber-50 p-4 font-bold text-amber-900">Komandas sastāva pievienošana un labošana ir slēgta. Sarakstus un drošības parakstu lapu varat apskatīt. Izmaiņām sazinieties ar organizatoru.</p>}
    <section className="mb-6 flex flex-wrap items-start justify-between gap-4 rounded-2xl border bg-white p-5">
      <div>
        <h2 className="text-xl font-black">Komandas pieteikums: {data.readiness.submitted ? "pabeigts" : "nav pabeigts"}</h2>
        <p className="mt-2 text-sm text-muted-foreground">Pēc sastāva izmaiņām pieteikums jāapstiprina vēlreiz.</p>
      ...
      <Button onClick={submitRoster} disabled={!data.rosterEditable || !data.readiness.canSubmit || data.readiness.submitted || submitting}>
...
function ParticipantEditor({ sports, person, registrations, editable, onSave }: {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
      ...
      <ParticipantForm sports={sports} participant={person} registrations={registrations}
